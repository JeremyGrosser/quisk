...
{	// Read sound samples from the ALSA soundcard.
	...
	if (avail == 0) {			// non-blocking: read available frames
		snd_pcm_delay(dev->handle, &avail);
		if (avail == 0)
			return 0;
...
#if DEBUG
		printf("Play underrun; nSamples %d\n", nSamples);
#endif
		//printf("State XRUN\n");
...
#endif
	if (snd_pcm_sw_params_set_start_threshold (handle, sware, dev->latency_frames * 7 / 8) < 0) {
...
{
	bzero(bufferz, sizeof(int) * SAMP_BUFFER_SIZE);
	...
	if (quisk_sound_state.err_msg[0])
		return;		// prior error
	// Open the alsa devices
	if ( ! pCapture->handle && pCapture->portaudio_index < 0)
		if (quisk_open_alsa_capture(pCapture))
			return;		// error
	if ( ! pPlayback->handle && pPlayback->portaudio_index < 0)
		if (quisk_open_alsa_playback(pPlayback))
			return;		// error
	if ( ! pMicCapture->handle && pMicCapture->portaudio_index < 0)
		if (quisk_open_alsa_capture(pMicCapture))
			return;		// error
	if ( ! pMicPlayback->handle && pMicPlayback->portaudio_index < 0)
		if (quisk_open_alsa_playback(pMicPlayback))
			return;		// error
	if (pCapture->handle && pCapture->portaudio_index < 0)
		snd_pcm_start((snd_pcm_t *)pCapture->handle);
	if (pMicCapture->handle && pMicCapture->portaudio_index < 0)
		snd_pcm_start((snd_pcm_t *)pMicCapture->handle);
}
...
{
	if (pCapture->handle && pCapture->portaudio_index < 0) {
		snd_pcm_drop((snd_pcm_t *)pCapture->handle);
		snd_pcm_close((snd_pcm_t *)pCapture->handle);
	}
	if (pPlayback->handle && pPlayback->portaudio_index < 0) {
		snd_pcm_drop((snd_pcm_t *)pPlayback->handle);
		snd_pcm_close((snd_pcm_t *)pPlayback->handle);
	}
	if (pMicCapture->handle && pMicCapture->portaudio_index < 0) {
		snd_pcm_drop((snd_pcm_t *)pMicCapture->handle);
		snd_pcm_close((snd_pcm_t *)pMicCapture->handle);
	}
	if (pMicPlayback->handle && pMicPlayback->portaudio_index < 0) {
		snd_pcm_drop((snd_pcm_t *)pMicPlayback->handle);
		snd_pcm_close((snd_pcm_t *)pMicPlayback->handle);
	}
}
